...
object ForValsImpl
{
  ...
  given _MainPublic
    ...
    : (
      ForNumericLiteranAndWordsAndIdentsImpl._MainPublic[VO_EOptConstructor.value.type , PAny ]
    ) )
  : _MainPublic[EOptConstructor, PAny]
  =
    new AnyRef with _MainPublic[EOptConstructor, PAny]
    {
      val SpclAfterDigestTupleOption: VO_EOptConstructor.value.type
      = valueOf 
      val iemiOpsR : rr.type
      = valueOf

      val given_ForNumericLiteranAndWordsAndIdentsImpl : given_ForNumericLiteranAndWordsAndIdentsImpl_impl.type
      = valueOf
    }
